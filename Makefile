...
all: build/xrp_$(NEW_VERSION).zip build/xrp_$(NEW_VERSION).zip.SHA512 build/xrp_$(NEW_VERSION).CHANGELOG
...
build/xrp_%.zip.SHA512: build/xrp_%.zip
	sha512sum $< >> $@
	sed -i 's#$(@D)/##' $@

build/xrp_%.CHANGELOG: build/xrp_%.zip.SHA512
	git rev-parse $* > /dev/null
	mkdir -p $(@D)/
