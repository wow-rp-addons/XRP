...
upload-curse-%: build/xrp-%.zip build/xrp-%.CHANGELOG
	curl -F "name=$*" -F "game_versions=$(shell curl -s http://wow.curseforge.com/game-versions.json | jq -r 'to_entries | map({id: .key, name: .value.name}) | .[] | select(.name | contains("$(shell echo $* | sed -e 's#\.[^\.]*$$##')")) | .id')" -F "file_type=$(shell echo $* | sed -e 's#^[^_]*$$#r#' -e 's#.*_alpha.*#a#' -e 's#.*_\(beta\|rc\).*#b#')" -F "change_log=<$(word 2,$^)" -F "change_markup_type=plain" -F "file=@$<" -H "X-API-Key: $(CURSE_API_KEY)" "http://wow.curseforge.com/addons/xrp/upload-file.json"
