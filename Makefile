...
build/xrp_%.zip:
	...
	cd $(@D)/tmp-$*/ && zip -q -D -X -l -9 -r $(CURDIR)/$@ xrp/ -x xrp/Makefile xrp/docs/CHANGES.txt
	rm -rf $(@D)/tmp-$*/
...
#upload-stormlord: upload-curse-$(NEW_VERSION)
...
.PHONY: all clean 
#upload upload-curse upload-stormlord upload-% upload-curse-% upload-stormlord-%
