...
if not _G.msp_RPAddOn then
	_G.msp_RPAddOn = GetAddOnMetadata("xrp", "Title")
else
...
local function filter_error(self, event, message)
	...
	if not dofilter then
		...
		-- 30 second timer between checks for offline characters. Try to not
		-- query offline characters higher up the chain as well, remember.
...
local function msprun_OnUpdate(self, elapsed)
	if next(self.request) then
		for character, fields in pairs(self.request) do
			--print(character..": "..fields)
			xrp.msp:Request(character, fields, msprun.safe[character])
			self.request[character] = nil
			msprun.safe[character] = nil
		end
	elseif next(msprun.send) then
...
msprun:RegisterEvent("ADDON_LOADED")
